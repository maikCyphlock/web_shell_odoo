...
class LogViewerController(http.Controller):
    ...
    @http.route("/web_shell/session/cleanup", type="json", auth="user")
    def cleanup_session(self):
        """
        Cleanup the current user's session to prevent memory leaks.
        Call this when leaving the DevTools page.
        """
        if not request.env.user.has_group("base.group_system"):
            return {"error": "Access Denied"}

        try:
            result = request.env["web.shell.console"].clear_user_session(
